...
// iOS-specific crash handler with file logging
#include <signal.h>
#include <sys/mman.h>

namespace CrashHandler
{
	static std::string s_crash_logs_path;
	static bool s_in_signal_handler_ios = false;

	// Signal-safe function to write to a file descriptor
	static void WriteToFd(int fd, const char* str)
	{
		if (fd < 0 || !str)
			return;
	...
	// Signal-safe function to write formatted integer as hex
	static void WriteHex(int fd, uintptr_t value)
	{
		char buf[64];
		int len = std::snprintf(buf, sizeof(buf), "0x%016lx", value);
		if (len > 0)
			::write(fd, buf, static_cast<size_t>(len));
	}

	// Signal-safe function to get current time
	static void GetTimeString(char* buf, size_t len)
	{
		time_t now = time(nullptr);
		struct tm* tm_info = localtime(&now);
		if (tm_info)
			std::strftime(buf, len, "%Y-%m-%d %H:%M:%S", tm_info);
		else
			std::snprintf(buf, len, "unknown");
	}

	// Signal-safe crash report writer
	static void WriteCrashReport(int signal, siginfo_t* siginfo, void* ctx)
	{
		if (s_crash_logs_path.empty())
			return;

		// Concatenate path: s_crash_logs_path + "/crash_report.txt"
		char crash_report_path[1024];
		std::snprintf(crash_report_path, sizeof(crash_report_path), "%s/crash_report.txt",
					  s_crash_logs_path.c_str());
		...
		// Write report header
		WriteToFd(fd, "====== iPSX2 Crash Report ======\n");

		// Timestamp
		char time_buf[64];
		GetTimeString(time_buf, sizeof(time_buf));
		char ts_line[128];
		std::snprintf(ts_line, sizeof(ts_line), "Timestamp: %s\n", time_buf);
		WriteToFd(fd, ts_line);
		...
		char sig_line[64];
		std::snprintf(sig_line, sizeof(sig_line), "Signal: %s (%d)\n", signal_name, signal);
		WriteToFd(fd, sig_line);

		// Fault address from siginfo
		if (siginfo)
		{
			WriteToFd(fd, "Fault Address: ");
			WriteHex(fd, reinterpret_cast<uintptr_t>(siginfo->si_addr));
			WriteToFd(fd, "\n");
		}

		// Program counter at crash time - arm64 specific
		...
		if (ctx)
		{
			...
			exception_pc = reinterpret_cast<void*>(uc->uc_mcontext->__ss.__pc);
#else
			exception_pc = reinterpret_cast<void*>(uc->uc_mcontext.pc);
#endif
		}
#endif
		...
		if (exception_pc)
		{
			WriteToFd(fd, "Program Counter: ");
			WriteHex(fd, reinterpret_cast<uintptr_t>(exception_pc));
			WriteToFd(fd, "\n");
		}
...
void CrashHandler::CrashSignalHandler(int signal, siginfo_t* siginfo, void* ctx)
{
	if (!s_in_signal_handler_ios)
	{
		s_in_signal_handler_ios = true;
...
void CrashHandler::SetWriteDirectory(std::string_view dump_directory)
{
	s_crash_logs_path = dump_directory;
}

void CrashHandler::WriteDumpForCaller()
{
	WriteCrashReport(0, nullptr, nullptr);
}
